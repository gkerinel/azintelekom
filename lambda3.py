...
dailylog_scality_bucket = 'dailylog-prd'  # Scality için log bucket
...
def log_latest_activity(last_uploaded_file, last_deleted_file):
    ...
    # Günlük log dosyasını Scality'ye kopyala
    try:
        scality_s3.put_object(Body=updated_content, Bucket=dailylog_scality_bucket, Key=log_key)
        print("Daily log successfully copied to Scality.")
    except Exception as e:
        print(f"Error copying daily log to Scality: {str(e)}")
